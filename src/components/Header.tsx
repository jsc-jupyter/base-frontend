...
function UserDropdown() {
  ...
  return (
    <Dropdown>
      <Dropdown.Toggle type="button" variant="outline-primary" className="mb-2">
        {displayName}
      </Dropdown.Toggle>
      <Dropdown.Menu className="w-100">
        <Dropdown.Item id="logout">
          <svg
            className="align-middle bi bi-box-arrow-right"
...
function Navigation({ prefix }: { prefix: string }) {
  const loggedInItems = (
    <>
      ...
      {authState.groups.includes('geant:dfn.de:fz-juelich.de:jsc:jupyter:workshop_instructors') && (
        <Nav.Item>
          <a
            id={`${prefix}workshop-manage-nav-item`}
            className="nav-link text-decoration-none"
  ...
  return (
    <Nav>
      ...
      <Nav.Item>
        <a
          id={`${jupyterHubData.prefix}docs-nav-item`}
          className="nav-link text-decoration-none"
          target="_blank"
          href="https://docs.jupyter.jsc.fz-juelich.de"
        >
          Documentation
        </a>
      </Nav.Item>
...
export default function Header() {
  return (
    <Navbar variant="light" expand="lg" className="bg-white pb-0">
      <Container fluid>
        <Navbar.Brand href="https://fz-juelich.de/jsc/en" target="_blank">
          <img
            ...
            className="p-3"
            width="335px"
            title="https://fz-juelich.de/jsc/en"
            alt="JSC"
        ...
        <div className="d-flex flex-column align-items-end ms-auto">
          <Navbar.Brand href="/">
            <img
              id="jupyter-logo"
              src={staticUrl('images', 'header', 'jupyterjsc.png')}
              height="60px"
              alt="Jupyter-JSC"
